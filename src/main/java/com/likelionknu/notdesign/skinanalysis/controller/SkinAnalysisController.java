...
public class SkinAnalysisController {
    ...
    @Operation(summary = "체험 피부 이미지 분석 결과 조회", description = "done 이면 결과 반환, 그 외(진행중/실패/없음)는 404")
    public GlobalResponse<DiaryResultDto> getDiaryResult(@PathVariable String requestId) {
        return GlobalResponse.ok(skinAnalysisService.getDiaryResult(requestId));
    }
}
